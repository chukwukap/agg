 ...
 */
describe("integration: router behaviour", () => {
  ...
  it("executes a single-leg route & deducts protocol fee", async () => {
  ...
  it("fails when mint continuity breaks", async () => {
    ...
    // Use async/await with try/catch to assert rejection
  ...
  it("fails when spent tokens exceed user_max_in", async () => {
  ...
  it("fails when net out < user_min_out", async () => {
  ...
  it("fails when fee vault mint differs from out mint", async () => {
    const other = await setupTokenAccounts();
  ...
  it("requires compute budget for long paths", async () => {
    const leg = buildDummyLeg(mint, 1_000n, 900n);
    const longPath = Array.from({ length: 25 }, () => ({ ...leg }));
  ...
  it("fails when too many remaining accounts provided", async () => {
  ...
  it("adapter whitelist rejects foreign owner", async () => {
    const foreignLeg = {
  ...
  it("fails when legs array is empty", async () => {
    let errorCaught = false;
    try {
      await program.methods
  ...
  it("fails when fee vault address != config fee_vault", async () => {
  ...
  it("invariant adapter whitelist rejects foreign owner", async () => {
    const invLeg = {
      ...buildDummyLeg(mint, 1000n, 900n),
